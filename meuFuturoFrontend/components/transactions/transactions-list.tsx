/**
 * Virtualized transactions list with optimal performance
 * Supports large datasets with smooth scrolling and selection
 */
...
const FixedSizeList = dynamic(() => import('react-window').then(mod => ({ default: mod.FixedSizeList })), {
  ssr: false,
  loading: () => <div className="h-[600px] flex items-center justify-center">Carregando lista...</div>
})
import { 
  AlertCircle, 
  Inbox, 
...
function VirtualListItem({ index, style, data }: VirtualListItemProps) {
  const { 
    transactions, 
    selectedIds, 
    onSelect, 
...
export function TransactionsList({
  ...
  const virtualListData = useMemo(() => ({
    ...
    onViewDetails,
    onDuplicate
  }), [transactions, selection.selectedIds, handleSelect, onEdit, onDelete, onViewDetails, onDuplicate])
